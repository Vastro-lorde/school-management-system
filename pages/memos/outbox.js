...
export default function MemoOutboxPage({ menu }) {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch('/api/memos?box=outbox').then(r=>r.json());
  ...
  return (
    <DashboardLayout menu={menu}>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-semibold">Memo Outbox</h1>
...
export async function getServerSideProps(ctx) {
  ...
  const ok = await permissionService.hasAccessToUrl(role, '/memos/outbox');
