...
export default function AdminDepartmentsPage({ menu }) {
  ...
  const createSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch('/api/admin/departments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
      const json = await res.json();
      if (!json.success) throw new Error(json.message || 'Failed');
  ...
  const startEdit = (item) => {
    setEditing(item);
    setForm({ name: item.name || '', code: item.code || '', description: item.description || '', active: !!item.active });
    setOpenEdit(true);
  };
  ...
  const editSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`/api/admin/departments/${editing._id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
      const json = await res.json();
      if (!json.success) throw new Error(json.message || 'Failed');
      setOpenEdit(false);
      setEditing(null);
  ...
  return (
    <DashboardLayout menu={menu}>
      ...
      <div className="mb-4">
        <button onClick={() => setOpenCreate(true)} className="bg-blue-600 text-white px-3 py-1 rounded">Add Department</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="overflow-auto">
          <table className="min-w-full text-sm">
            <thead><tr className="text-left"><th className="p-2">Name</th><th className="p-2">Code</th><th className="p-2">Description</th><th className="p-2">Active</th><th className="p-2">Actions</th></tr></thead>
            <tbody>
              {items.map(i => (
      ...
      <Modal open={openCreate} onClose={() => setOpenCreate(false)} title="Add Department" footer={null}>
      ...
      <Modal open={openEdit} onClose={() => setOpenEdit(false)} title="Edit Department" footer={null}>
...
export async function getServerSideProps(context) {
  ...
  const ok = await permissionService.hasAccessToUrl(role, '/admin/departments');
