...
export default function AdminRolesPage({ menu }) {
  ...
  const editSubmit = async (e) => {
    e.preventDefault();
    try {
      const body = { description: form.description, active: form.active };
      // allow rename if not system
      if (!editing.system) body.name = form.name;
      const res = await fetch(`/api/admin/roles/${editing._id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const json = await res.json();
      if (!json.success) throw new Error(json.message || 'Failed');
      setOpenEdit(false);
      setEditing(null);
  ...
  const remove = async (id) => {
    ...
    if (item?.system) return alert('Cannot delete a system role');
  ...
  return (
    <DashboardLayout menu={menu}>
      ...
      <div className="mb-4">
        <button onClick={() => setOpenCreate(true)} className="bg-blue-600 text-white px-3 py-1 rounded">Add Role</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="overflow-auto">
          <table className="min-w-full text-sm">
            <thead><tr className="text-left"><th className="p-2">Name</th><th className="p-2">Description</th><th className="p-2">Active</th><th className="p-2">System</th><th className="p-2">Actions</th></tr></thead>
            <tbody>
              {items.map(i => (
                <tr key={i._id} className="border-t">
                  ...
                  <td className="p-2">{i.system ? 'Yes' : 'No'}</td>
      ...
      <Modal open={openCreate} onClose={() => setOpenCreate(false)} title="Add Role" footer={null}>
        <form onSubmit={createSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <input className="border rounded px-2 py-1 md:col-span-2" placeholder="Name (e.g., teacher)" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
      ...
      <Modal open={openEdit} onClose={() => setOpenEdit(false)} title="Edit Role" footer={null}>
        <form onSubmit={editSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <input className="border rounded px-2 py-1 md:col-span-2" placeholder="Name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} disabled={!!editing?.system} />
          <textarea className="border rounded px-2 py-1 md:col-span-2" placeholder="Description" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
          <label className="flex items-center gap-2 md:col-span-2">
            <input type="checkbox" checked={form.active} onChange={e => setForm({ ...form, active: e.target.checked })} /> Active
          </label>
...
export async function getServerSideProps(context) {
  ...
  const ok = await permissionService.hasAccessToUrl(role, '/admin/roles');
