...
export default function AdminStudentPositionsPage({ menu }) {
  const [term, setTerm] = useState('');
  const [results, setResults] = useState([]);
  const [positions, setPositions] = useState([]);

  async function search() {
    const res = await fetch(`/api/admin/lookup?type=student&q=${encodeURIComponent(term)}`).then(r=>r.json());
    if (res?.success) setResults(res.value||[]); else alert(res?.message||'Failed');
  }

  async function loadPositions() {
    const res = await fetch('/api/admin/positions').then(r=>r.json());
  ...
  async function assign(studentProfileId, positionId) {
    ...
    const res = await fetch(`/api/admin/students/${studentProfileId}/position`, { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ positionId }) }).then(r=>r.json());
    if (res?.success) alert('Assigned'); else alert(res?.message||'Failed');
  }

  return (
    <DashboardLayout menu={menu}>
      <h1 className="text-2xl font-semibold mb-4">Student Positions</h1>
...
export async function getServerSideProps(ctx) {
  ...
  const ok = await permissionService.hasAccessToUrl(role, '/admin/student-positions');
