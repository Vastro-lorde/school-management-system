...
export default function AdminStaffsPage({ menu }) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  useEffect(() => { (async () => { const res = await fetch('/api/admin/staffs/stats').then(r => r.json()); if (res?.success) setData(res.value); setLoading(false); })(); }, []);
  const maxDept = useMemo(() => Math.max(0, ...((data?.byDepartment||[]).map(x=>x.count))), [data]);
  const maxGender = useMemo(() => Math.max(0, ...((data?.byGender||[]).map(x=>x.count))), [data]);
  return (
    <DashboardLayout menu={menu}>
      <h1 className="text-2xl font-semibold mb-4">Staffs</h1>
      {loading ? <div className="py-6 text-center opacity-80">Loading...</div> : (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <Stat label="Total staff" value={data.total} />
          </div>
              ...
              <div className="space-y-2">
                {(data?.byDepartment||[]).map(r => (
                  <div key={r._id} className="space-y-1">
                    <div className="flex justify-between text-xs opacity-70"><span>{r._id}</span><span>{r.count}</span></div>
                    <div className="h-2 bg-gray-200 rounded"><div className="h-2 bg-blue-600 rounded" style={{ width: `${maxDept ? Math.round((r.count/maxDept)*100) : 0}%` }} /></div>
                  </div>
                ))}
              </div>
            </div>
...
export async function getServerSideProps(context) {
  ...
  const ok = await permissionService.hasAccessToUrl(role, '/admin/staffs');
